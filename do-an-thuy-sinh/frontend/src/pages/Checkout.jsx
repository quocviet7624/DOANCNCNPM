...
// ─── Tính phí ship từ config động (lấy từ server/localStorage) ───────────────
const calcShippingFromConfig = (province, subtotal, shippingConfig) => {
    if (!shippingConfig || !province)
    ...
    const zoneInfo = zoneId !== undefined ? zones?.[zoneId] : null;
    const originalFee = zoneInfo?.fee ?? 0;

    if (zoneId === undefined)
        return { fee: 0, isFree: false, zoneLabel: 'Chưa xác định', originalFee: 0 };
    if (!zoneInfo)
        return { fee: 0, isFree: false, zoneLabel: 'Vùng không tồn tại', originalFee: 0 };
    if (subtotal >= freeShipThreshold)
        return { fee: 0, isFree: true, zoneLabel: zoneInfo.label, originalFee };

    return { fee: zoneInfo.fee, isFree: false, zoneLabel: zoneInfo.label, originalFee };
};
// ─────────────────────────────────────────────────────────────────────────────
...
const Checkout = () => {
    ...
    // ── SHIPPING CONFIG ĐỘNG (đồng bộ với AdminShipping) ─────────────────────
    const [shippingConfig, setShippingConfig] = useState(() => {
        // Dùng cache localStorage trước (AdminShipping lưu vào đây sau mỗi lần save)
        try {
    ...
    // Lắng nghe event 'shippingConfigChange' do AdminShipping dispatch sau khi lưu
    useEffect(() => {
    ...
    // Computed từ config động
    const PROVINCE_LIST = shippingConfig
        ? Object.keys(shippingConfig.provinceZoneMap).sort((a, b) => a.localeCompare(b, 'vi'))
        : [];
    const SHIPPING_ZONES = shippingConfig?.zones ?? {};
    const FREE_SHIP_THRESHOLD = shippingConfig?.freeShipThreshold ?? 0;
    ...
    useEffect(() => {
        ...
        if (list.length === 0 && (user.address || user.phone)) {
            const migrated = [{
    ...
    // ── TÍNH TOÁN TIỀN (dùng config động) ────────────────────────────────────
    const subtotal = cartItems.reduce((s, i) => s + i.price * i.quantity, 0);
    const discountAmount = appliedVoucher?.discountAmount || 0;
    const shippingInfo = calcShippingFromConfig(selectedProvince, subtotal, shippingConfig);
    ...
    const submitOrder = async () => {
        ...
        try {
            ...
            if (appliedVoucher?.code) {
                try {
                    await axios.post(`http://localhost:5000/api/vouchers/confirm-use/${appliedVoucher.code}`);
                } catch { }
            }
    ...
    const ShippingBadge = () => {
        ...
        if (!shippingConfig) return (
            <span style={{ color: '#aaa', fontSize: 13 }}>Đang tải...</span>
        );
        ...
        if (shippingInfo.zoneLabel === 'Vùng không tồn tại' || shippingInfo.zoneLabel === 'Chưa xác định') return (
            <span style={{ color: '#aaa', fontSize: 13 }}>Chưa có phí ship cho khu vực này</span>
        );
        return (
                    ...
                    <Col xs={24} lg={10}>
                        <Card title="Đơn hàng của bạn" style={{ position: 'sticky', top: 20 }}>
                            ...
                            <div style={{ padding: '4px 0' }}>
                                ...
                                {selectedProvince && !shippingInfo.isFree && subtotal > 0 && FREE_SHIP_THRESHOLD > 0 && subtotal < FREE_SHIP_THRESHOLD && (
                                    <div style={{
                            ...
                            <Button
                                ...
                                disabled={isAddingNew || savedAddresses.length === 0 || !selectedProvince || !shippingConfig}
                                style={{
                                    height: 50,
                            ...
                            {/* Bảng phí giao hàng (từ config động) */}
                            {Object.keys(SHIPPING_ZONES).length > 0 && (
